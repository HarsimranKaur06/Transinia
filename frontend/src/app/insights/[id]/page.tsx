...
// Sample insight data (fallback for mock IDs)
const sampleInsight: Insight = {
  id: "1",
  title: "Product Planning Meeting",
  date: "September 10, 2025",
  summary:
    "The team discussed the Q4 roadmap and prioritized features for the next release. Key decisions were made about resource allocation and timeline adjustments to accommodate the new security requirements.",
  actionItems: [
...
export default function InsightPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        } else if (insightId.startsWith("mock_")) {
          // Fallback for mock
          setInsight({ ...sampleInsight, id: insightId });
        } else {
          setError(result.message || "Failed to load meeting data.");
        }
      } catch {
  ...
  const downloadMinutes = () => {
    ...
    const markdownContent =
      ...
      `## Action Items\n\n${insight.actionItems
        .map((item) => `- [${item.completed ? "x" : " "}] ${item.text}${item.assignee ? ` (Assigned to: ${item.assignee})` : ""}`)
        .join("\n")}\n\n` +
        ...
        <header className="border-b">
          <div className="container mx-auto flex h-16 items-center justify-between px-4">
            ...
            <nav className="ml-auto flex gap-4 sm:gap-6">
              <Link href="/transcripts" className="text-sm font-medium hover:underline underline-offset-4">
                Transcripts
              </Link>
            </nav>
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2">
            ...
            {insight.duration && insight.duration.toLowerCase() !== "unknown" && (
        ...
        {toast && (
          <div
            className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-md shadow-md ${
              toast.type === "success"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
              ...
              <ul className="space-y-3">
                {insight.actionItems.map((item) => (
                  <li key={item.id} className="flex items-start">
                    ...
                    >
                      <div className={`h-2 w-2 rounded-full ${item.completed ? "bg-green-500" : "bg-neutral-900"}`} />
                    </div>
                    <div>
                      <p className={`text-neutral-700 ${item.completed ? "text-neutral-400" : ""}`}>{item.text}</p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {item.assignee && <span className="text-sm text-neutral-500">Assigned to: {item.assignee}</span>}
                        {item.priority && (
                          <span
                        ...
                        {item.completed && <span className="text-xs px-2 py-1 rounded-md bg-green-100 text-green-700">Completed</span>}
                      </div>
                    </div>
                  </li>
                ))}
            ...
            <div className="bg-white p-6 rounded-lg border">
              ...
              <p className="text-xs text-neutral-500 mt-3">Note: Meeting data is automatically saved to S3</p>
            </div>
          </div>
        </div>
      </div>
